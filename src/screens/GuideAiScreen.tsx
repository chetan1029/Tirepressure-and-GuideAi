...
import GuideAiFlatList from '../components/GuideAiFlatList';
import GuideAiSearchCard from '../components/GuideAiSearchCard';
import {FONTFAMILY, FONTSIZE, SPACING} from '../theme/theme';

const GuideAiScreen = ({route, navigation}: any) => {
  // State
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      await fetchGuideAiItems(type, UserDetail);
      setLoading(false);
    };

    fetchData();
  }, [fetchGuideAiItems, UserDetail, type]);
  ...
  return (
    <View
      ...
      {/* App Header */}
      <HeaderBar title={t('tireGuideAi')} themeColor={themeColor} />

      {/* Search Input */}
      <SearchBar
        searchText={searchText}
        searchList={searchGuideAiList}
        setSearchText={setSearchText}
        resetSearch={resetGuideAiSearch}
        themeColor={themeColor}
        placeholder={t('searchTireGuideAi')}
      />
      {type == 'general' ? (
        <TouchableOpacity
          onPress={() => {
            navigation.navigate('GuideAiScreen', {type: 'user-history'});
          }}
        ...
        <TouchableOpacity
          onPress={() => {
            navigation.navigate('GuideAiScreen', {type: 'general'});
          }}
          style={styles.SearchHistroy}>
          <Text
            style={[styles.HistoryTitle, {color: themeColor.primaryTextFocus}]}>
